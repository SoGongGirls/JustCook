...
public abstract class CameraActivity extends AppCompatActivity
    ...
    public void onPreviewFrame(final byte[] bytes, final Camera camera) {
        ...
        try {
            // 해상도가 알려지면, 스토리지 비트맵을 한 번 초기화합니다.
            if (rgbBytes == null) {
    ...
    public void onImageAvailable(final ImageReader reader) {
        // onPreviewSizeChosen에서 사이즈를 선택할 때까지 기다려야 합니다.
    ...
    // 디바이스가 필요한 하드웨어 레벨 또는 그 이상을 지원할 경우, true를 리턴합니다.
    private boolean isHardwareLevelSupported(
        ...
        // deviceLevel이 LEGACY가 아니면, 숫자 정렬을 사용할 수 있습니다.
        return requiredLevel <= deviceLevel;
    }
    ...
    private String chooseCamera() {
        ...
        try {
            for (final String cameraId : manager.getCameraIdList()) {
                ...
                // 전체 지원이 없는 내장 카메라의 경우, camera1 API로 돌아갑니다.
                // 이렇게 하면 camera2 API를 사용할 때, 미리보기가 왜곡되거나 파손되는 기존 상황에 도움이 됩니다.
                useCamera2API =
    ...
    protected void fillBytes(final Plane[] planes, final byte[][] yuvBytes) {
        // 가변 행 보폭 때문에 yuv 평면의 실제 필요한 크기를 미리 알 수 없습니다.
